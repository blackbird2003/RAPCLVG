...
from .project_store import InvalidProjectError
from .runner import NotebookRunner


class BackgroundJobManager:
    def __init__(self, runner: NotebookRunner, *, max_workers: int | None = None) -> None:
        self.runner = runner
        self.max_workers = max(1, int(max_workers or os.getenv("VIDEOGEN_NOTEBOOK_MAX_WORKERS", "30")))
        self.executor = ThreadPoolExecutor(max_workers=self.max_workers, thread_name_prefix="videogen-notebook")
        self._lock = Lock()
