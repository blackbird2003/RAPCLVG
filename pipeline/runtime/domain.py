...
MIN_SMOOTH_REFERENCE_SECONDS = 1.8
DEFAULT_NON_CUT_GENERATION_MODE = "smooth"
MAX_REFERENCE_IMAGES = 9
VISUAL_SELECTION_MODES = {"greedy_coverage", "static_top_k", "naive_top_k", "sink_recent_memory", "none"}
...
def default_settings() -> Dict[str, Any]:
    return {
        "generation": {
            ...
            "audio": True,
            "auto_run_step_review_delay_seconds": 0,
...
def validate_settings(settings: Dict[str, Any]) -> Dict[str, Any]:
    ...
    delay = _non_negative_int(
        generation.get("auto_run_step_review_delay_seconds", 0),
        "auto_run_step_review_delay_seconds",
    )
    generation["auto_run_step_review_delay_seconds"] = delay if delay in {0, 10, 30, 60, 120} else 0
    ...
    if smooth_reference_seconds <= 0:
        smooth_reference_seconds = DEFAULT_SMOOTH_REFERENCE_SECONDS
    generation["smooth_reference_seconds"] = min(
        max(smooth_reference_seconds, MIN_SMOOTH_REFERENCE_SECONDS),
        10.0,
    )
...
def _normalize_predefined_references(value: Any, scene_num: int, shot_num: int) -> List[Dict[str, Any]]:
    ...
    for index, item in enumerate(value, start=1):
        ...
        if not media_path:
            raise StoryValidationError(
                f"Scene {scene_num} / Shot {shot_num} predefined reference {index} missing media path"
            )
        if media_type not in {"image", "audio", "video"}:
            raise StoryValidationError(
                f"Scene {scene_num} / Shot {shot_num} predefined reference {index} has unsupported media_type: {media_type}"
            )
        reference = {
