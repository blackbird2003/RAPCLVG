...
PREDEFINED_REFERENCE_EXTENSIONS = {
    "image": {".jpg", ".jpeg", ".png", ".bmp", ".webp", ".tiff", ".gif"},
    "audio": {".mp3", ".wav", ".m4a", ".aac", ".ogg", ".flac"},
    "video": {".mp4", ".mov", ".webm", ".mkv", ".avi"},
}
MAX_PREDEFINED_REFERENCE_MEDIA = {"image": MAX_REFERENCE_IMAGES, "audio": 3, "video": 3}
...
class ProjectStore:
    ...
    def save_project(
        ...
        *,
        ...
        for update in shot_updates:
            ...
            _validate_predefined_reference_limits(
                next_inputs["predefined_references"],
                reserve_smooth_video=mode == "smooth",
            )
            current_inputs = {
                **inputs,
    ...
    def add_predefined_reference(
        ...
        *,
        ...
        if shot.get("state", {}).get("status") not in {"draft", "failed", "interrupted"}:
            raise InvalidProjectError(f"Shot {shot_id} is locked in state {shot.get('state', {}).get('status')}")
        if not data:
            raise InvalidProjectError("Uploaded reference media is empty")
        media_type = _normalize_predefined_media_type(media_type)
        suffix = Path(filename or "").suffix.lower() or ".jpg"
        if suffix not in PREDEFINED_REFERENCE_EXTENSIONS[media_type]:
            raise InvalidProjectError(f"Unsupported predefined {media_type} extension: {suffix}")
        ...
        record = {
            ...
            "media_path": _relative(self.project_dir(project_id), target),
            "label": str(label or "").strip() or Path(filename or asset_id).stem,
            "guidance": _normalize_textarea_text(guidance),
        }
        if media_type == "image":
            record["image_path"] = record["media_path"]
        existing.append(record)
        _validate_predefined_reference_limits(
            existing,
            reserve_smooth_video=str(shot.get("inputs", {}).get("generation_mode") or "default") == "smooth",
        )
    ...
    def apply_predefined_references_to_subsequent(
        self,
        project_id: str,
        shot_id: str,
    ) -> Dict[str, Any]:
        bundle = self.get_project(project_id)
        project = bundle["project"]
        if project.get("status") in {"running", "waiting_next_step", "waiting_retry"}:
            raise InvalidProjectError("Cannot apply predefined references while the project is running")
        shots = sorted(bundle["shots"], key=lambda item: int(item.get("order_index", 0)))
        source_index = next(
            (index for index, shot in enumerate(shots) if shot.get("shot_id") == shot_id),
            None,
        )
        if source_index is None:
            raise NotFoundError(f"Shot not found: {shot_id}")
        source = shots[source_index]
        source_refs = _normalize_predefined_references_update(
            source.get("inputs", {}).get("predefined_references", []),
            source.get("inputs", {}).get("predefined_references", []),
        )
        targets = shots[source_index + 1 :]
        for target in targets:
            target_status = target.get("state", {}).get("status")
            if target_status not in {"draft", "failed", "interrupted"}:
                raise InvalidProjectError(
                    f"Shot {target['shot_id']} is locked in state {target_status}"
                )

        project_dir = self.project_dir(project_id)
        manifest_assets: Dict[str, Dict[str, Any]] = {}
        for target in targets:
            target_id = target["shot_id"]
            target_mode = str(target.get("inputs", {}).get("generation_mode") or "default")
            _validate_predefined_reference_limits(
                source_refs,
                reserve_smooth_video=target_mode == "smooth",
            )
            copied: List[Dict[str, Any]] = []
            for index, reference in enumerate(source_refs, start=1):
                source_path = project_dir / str(reference.get("media_path") or "")
                if not source_path.is_file() and reference.get("asset_id"):
                    source_path = self.asset_path(project_id, str(reference["asset_id"]))
                if not source_path.is_file():
                    raise InvalidProjectError(
                        f"Predefined reference {reference.get('id')} media not found: {source_path}"
                    )
                media_type = _normalize_predefined_media_type(reference.get("media_type") or "image")
                suffix = source_path.suffix.lower() or ".jpg"
                if suffix not in PREDEFINED_REFERENCE_EXTENSIONS[media_type]:
                    raise InvalidProjectError(f"Unsupported predefined {media_type} extension: {suffix}")
                reference_id = f"pref-{index:04d}"
                asset_id = _predefined_asset_id(target_id, reference_id)
                target_path = project_dir / "assets" / "predefined_references" / f"{asset_id}{suffix}"
                ...
                item = {
                    ...
                    "asset_id": asset_id,
                    "media_type": media_type,
                    "media_path": _relative(project_dir, target_path),
                    "label": reference.get("label") or source_path.stem,
                    "guidance": _normalize_textarea_text(reference.get("guidance") or ""),
                }
                if media_type == "image":
                    item["image_path"] = item["media_path"]
                copied.append(item)
                manifest_assets[asset_id] = _predefined_asset_record(
                    project_dir, target_path, asset_id, target_id, item
                )
            target["inputs"]["predefined_references"] = copied
            target["state"]["updated_at"] = _now()
            self.write_shot(project_id, target)
        if manifest_assets:
            self.update_asset_manifest(project_id, manifest_assets)
        project["updated_at"] = _now()
        write_json_atomic(project_dir / "project.json", project)
        self._write_story_design(project_id)
        return self.get_project(project_id)
        ...
        data = {
            ...
            "inputs": {
                ...
                "predefined_references": self._copy_predefined_references(
                    ...
                    reserve_smooth_video=str(shot.get("generation_mode") or "default") == "smooth",
                ),
            },
            "state": {
                "status": "draft",
    ...
    def _hydrate_predefined_references(self, project_id: str, shot: Dict[str, Any]) -> None:
        ...
        for reference in references:
            ...
            if asset_id:
                ...
                continue
            url = self._project_file_url(
                project_id,
                project_dir,
                reference.get("media_path") or reference.get("image_path"),
            )
            if url:
                reference["media_url"] = url

    def _copy_predefined_references(
        self,
        project_dir: Path,
        shot_id: str,
        references: List[Dict[str, Any]],
        *,
        reserve_smooth_video: bool = False,
    ) -> List[Dict[str, Any]]:
        normalized = _normalize_imported_predefined_references(references)
        _validate_predefined_reference_limits(normalized, reserve_smooth_video=reserve_smooth_video)
        copied: List[Dict[str, Any]] = []
        assets: Dict[str, Dict[str, Any]] = {}
        for index, reference in enumerate(normalized, start=1):
            source = _resolve_import_media_path(reference["media_path"])
            if not source.is_file():
                raise InvalidProjectError(f"Predefined reference {reference['media_type']} not found: {source}")
            suffix = source.suffix.lower() or ".jpg"
            media_type = _normalize_predefined_media_type(reference["media_type"])
            if suffix not in PREDEFINED_REFERENCE_EXTENSIONS[media_type]:
                raise InvalidProjectError(f"Unsupported predefined {media_type} extension: {source}")
            reference_id = f"pref-{index:04d}"
            asset_id = _predefined_asset_id(shot_id, reference_id)
            ...
            item = {
                ...
                "asset_id": asset_id,
                "media_type": media_type,
                "media_path": _relative(project_dir, target),
...
def _normalize_imported_predefined_references(value: Any) -> List[Dict[str, Any]]:
    ...
    for index, item in enumerate(value, start=1):
        ...
        media_type, media_path = _predefined_media_location(item)
        if not media_path:
            raise InvalidProjectError(f"predefined reference {index} missing media path")
        media_type = _normalize_predefined_media_type(media_type)
        reference = {
            "media_type": media_type,
    ...
    _validate_predefined_reference_limits(references)
    return references
...
def _normalize_predefined_references_update(value: Any, existing: Any) -> List[Dict[str, Any]]:
        ...
        normalized.append(
            {
                ...
                "media_type": _normalize_predefined_media_type(base.get("media_type") or "image"),
                "media_path": str(base.get("media_path") or base.get("image_path") or item.get("media_path") or item.get("image_path") or "").strip(),
                "label": str(item.get("label") or "").strip(),
    ...
    for reference in normalized:
        if reference["media_type"] == "image":
            reference["image_path"] = reference["media_path"]
    _validate_predefined_reference_limits(normalized)
    return normalized
...
def _resolve_import_media_path(value: str) -> Path:
...
def _predefined_asset_record(
    ...
    return {
        "kind": _normalize_predefined_media_type(reference.get("media_type") or "image"),
...
def _export_predefined_references(project_dir: Path, references: Any) -> List[Dict[str, Any]]:
    ...
    for item in references or []:
        if not isinstance(item, dict):
            continue
        media_type = _normalize_predefined_media_type(item.get("media_type") or "image")
        media_path = str(item.get("media_path") or item.get("image_path") or "").strip()
        if media_path and project_dir.is_absolute():
            media_path = str((project_dir / media_path).resolve())
        exported = {
            "media_type": media_type,
            "media_path": media_path,
            "label": str(item.get("label") or "").strip(),
        ...
        if media_type == "image":
            exported["image_path"] = media_path
        result.append(exported)
    return result


def _predefined_media_location(item: Dict[str, Any]) -> tuple[str, str]:
    media_type = str(item.get("media_type") or "").strip().lower()
    if not media_type:
        for candidate in ("image", "audio", "video"):
            if item.get(f"{candidate}_path"):
                media_type = candidate
                break
    if not media_type:
        media_type = "image"
    media_path = str(
        item.get(f"{media_type}_path")
...
def _normalize_predefined_media_type(value: Any) -> str:
    media_type = str(value or "image").strip().lower()
    if media_type not in PREDEFINED_REFERENCE_EXTENSIONS:
        raise InvalidProjectError(f"Unsupported predefined media type: {media_type}")
    return media_type


def _validate_predefined_reference_limits(
    references: List[Dict[str, Any]],
    *,
    reserve_smooth_video: bool = False,
) -> None:
    counts = {media_type: 0 for media_type in PREDEFINED_REFERENCE_EXTENSIONS}
    for reference in references:
        if not isinstance(reference, dict):
            continue
        counts[_normalize_predefined_media_type(reference.get("media_type") or "image")] += 1
    for media_type, maximum in MAX_PREDEFINED_REFERENCE_MEDIA.items():
        reserved = 1 if media_type == "video" and reserve_smooth_video else 0
        if counts[media_type] + reserved > maximum:
            noun = "Smooth continuation video plus predefined videos" if reserved else f"Predefined {media_type} references"
            raise InvalidProjectError(f"{noun} exceed Seedance's {maximum}-{media_type} limit")
