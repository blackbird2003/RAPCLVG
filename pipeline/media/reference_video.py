...
def extract_video_tail(
    ...
    *,
    ...
    tail_frames = min(frame_count, max(1, math.ceil(fps * seconds)))
...
class CloudflareTunnelPublisher:
    def __init__(
        ...
        *,
        ...
        self.serve_dir = Path(serve_dir or os.getenv("VIDEOGEN_REFERENCE_VIDEO_SERVE_DIR") or Path(tempfile.gettempdir()) / "videogen_reference_video_tunnel")
        self.cloudflared_bin = cloudflared_bin or os.getenv("VIDEOGEN_CLOUDFLARED_BIN") or _cloudflared_executable()
...
def configured_reference_video_publisher() -> ReferenceVideoPublisher:
    ...
    provider = os.getenv("VIDEOGEN_REFERENCE_VIDEO_PUBLISHER", "").strip().lower()
    if provider in {"cloudflare", "cloudflare_tunnel", "trycloudflare"}:
        if _CLOUDFLARE_TUNNEL_PUBLISHER is None:
            _CLOUDFLARE_TUNNEL_PUBLISHER = CloudflareTunnelPublisher(
                startup_timeout=float(os.getenv("VIDEOGEN_CLOUDFLARE_TUNNEL_TIMEOUT", "60")),
            )
        return _CLOUDFLARE_TUNNEL_PUBLISHER
    if provider == "tmpfiles":
        return TmpfilesPublisher(
            timeout=float(os.getenv("VIDEOGEN_REFERENCE_VIDEO_TIMEOUT", "60")),
            retries=int(os.getenv("VIDEOGEN_REFERENCE_VIDEO_RETRIES", "10")),
        )
    raise RuntimeError(
        "Smooth mode requires VIDEOGEN_REFERENCE_VIDEO_PUBLISHER=cloudflare_tunnel, tmpfiles, "
...
def _cloudflared_executable() -> str:
    for candidate in (
        os.getenv("VIDEOGEN_CLOUDFLARED_BIN"),
        shutil.which("cloudflared"),
        "/tmp/videogen_tools/cloudflared",
    ):
        if candidate and Path(candidate).is_file() and os.access(candidate, os.X_OK):
            return str(candidate)
    target = Path(tempfile.gettempdir()) / "videogen_tools" / "cloudflared"
    target.parent.mkdir(parents=True, exist_ok=True)
    if target.is_file() and os.access(target, os.X_OK):
        return str(target)
    url = os.getenv(
        "VIDEOGEN_CLOUDFLARED_DOWNLOAD_URL",
