...
def _parse_args():
    ...
    parser.add_argument("--prompt_retrieval", action="store_true", help="Enable prompt-aware retrieval for cut shots.")
    parser.add_argument("--retrieval_top_k", type=int, default=2, help="Number of prompt-retrieved keyframes to reserve in the memory bank.")
    parser.add_argument("--retrieval_frame_weight", type=float, default=0.7, help="Weight for CLIP text-image keyframe relevance.")
    parser.add_argument("--retrieval_video_weight", type=float, default=0.3, help="Weight for current prompt vs source-shot prompt relevance.")
    parser.add_argument("--retrieval_min_score", type=float, default=None, help="Optional minimum fused retrieval score.")
    parser.add_argument("--retrieval_clip_device", type=str, default="cpu", choices=["cpu", "cuda"], help="Device for CLIP retrieval scoring.")
...
def main(args):
    ...
    for scene in story_script["scenes"]:
        ...
        for i, prompt in enumerate(scene["video_prompts"]):
            ...
            all_memory = sorted(glob.glob(f"{args.output_dir}/*keyframe*.jpg"))
            if args.prompt_retrieval and scene["cut"][i]:
                memory_query = get_memory_query(scene, i, prompt)
                memory_bank, retrieval_hits = build_prompt_aware_memory_bank(
                    ...
                    frame_weight=args.retrieval_frame_weight,
                    video_weight=args.retrieval_video_weight,
                    min_score=args.retrieval_min_score,
                    clip_device=args.retrieval_clip_device,
                    logger=logging,
                )
                logging.info(
                    "Prompt-aware memory query: %s | selected=%d | memory=%d/%d",
                    memory_query,
                    len(retrieval_hits),
                    len(memory_bank),
                    len(all_memory),
                ...
                memory_bank = build_default_memory_bank(
                    all_memory,
                    max_memory_size=args.max_memory_size,
                    fix=args.fix,
                )
