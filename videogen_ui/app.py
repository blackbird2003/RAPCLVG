...
from pipeline.runtime.domain import StoryValidationError
from pipeline.runtime.project_store import InvalidProjectError, NotFoundError, ProjectStore
...
def create_app(workspace: Optional[str] = None, runner_backend: Optional[str] = None) -> FastAPI:
    store = ProjectStore(workspace or os.getenv("VIDEOGEN_WORKSPACE", ".runtime/videogen_ui"))
    store.recover_interrupted_runs()
    runner = create_runner(
        store,
        runner_backend or os.getenv("VIDEOGEN_RUNNER", "fake"),
    )
    app = FastAPI(title="videogen_ui")
    ...
    async def add_predefined_reference(
        ...
        try:
            uploaded = media_file or image_file
            if uploaded is None:
                raise InvalidProjectError("Select a reference image, audio, or video file")
            store.add_predefined_reference(
                project_id,
                shot_id,
                filename=uploaded.filename or "reference",
    ...
    @app.post("/projects/{project_id}/shots/{shot_id}/predefined-references/apply-subsequent")
    async def apply_predefined_references_to_subsequent(project_id: str, shot_id: str):
        try:
            store.apply_predefined_references_to_subsequent(project_id, shot_id)
        except (NotFoundError, InvalidProjectError, ValueError) as exc:
            raise HTTPException(status_code=400, detail=str(exc)) from exc
        return _redirect(f"/projects/{project_id}#{shot_id}")
...
def _settings_update_from_form(form: Any) -> dict[str, Any]:
    return {
        ...
        "generation": {
            ...
            "auto_run_step_review_delay_seconds": form.get("auto-run-step-review-delay", "0"),
